    ...
    private var metricsGrid: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            LazyVGrid(columns: [
                ...
                MetricCell(label: "Stations heard", value: "\(health.metrics.totalStations)")
                MetricCell(label: "Active (10m)", value: "\(health.metrics.activeStations)")
                MetricCell(label: "Total packets", value: formatNumber(health.metrics.totalPackets))
                MetricCell(label: "Packets/min", value: String(format: "%.1f", health.metrics.packetRate))
                MetricCell(label: "Main cluster", value: "\(Int(health.metrics.largestComponentPercent))%")
                MetricCell(label: "Top relay share", value: "\(Int(health.metrics.topRelayConcentration))%")
            }
        }
    }
