...
final class NonAXDPDataDeliveryTests: XCTestCase {
    ...
    /// A peer sends AXDP data first, then plain text.
    /// The plain text MUST be delivered to the terminal.
    func testPlainTextAfterAXDPDataMustBeDelivered() async {
        await MainActor.run {
            var receivedLines: [String] = []
            ...
            // STEP 2: Send plain text (should be delivered despite AXDP first)
    ...
    /// Test multiple alternations between AXDP and plain text
    func testAlternatingAXDPAndPlainText() async {
        await MainActor.run {
            ...
            // Alternating: AXDP -> Plain -> AXDP -> Plain -> Plain
...
final class AXDPReassemblyFlagManagementTests: XCTestCase {
    ...
    /// Test that the flag IS CLEARED when non-AXDP data arrives from a peer that was in reassembly
    func testFlagClearedOnNonAXDPFromPeerInReassembly() async {
        await MainActor.run {
            let sessionManager = AX25SessionManager()
            sessionManager.localCallsign = AX25Address(call: "TEST", ssid: 1)
            ...
            // Step 2: Send plain text (should clear flag)
            viewModel.sessionManager.onDataReceived?(session, Data("Plain text\r\n".utf8))
            
            // Flag should now be CLEARED
            XCTAssertFalse(
                viewModel.isPeerInAXDPReassembly(peerKey),
                "Flag should be CLEARED after receiving non-AXDP data"
            )
        }
    }
...
final class ProtocolSwitchingTests: XCTestCase {
    ...
    /// Test rapid repeated switching: AXDP → Plain → AXDP → Plain → ... (20 times)
    func testRapidRepeatedProtocolSwitching() async {
        await MainActor.run {
            ...
            // Rapid alternation: 20 cycles of AXDP → Plain
            for i in 0..<20 {
                // AXDP message
    ...
    /// Test consecutive AXDP messages followed by plain text
    ...
    /// Test consecutive plain text messages, then AXDP, then plain text again
    func testPlainThenAXDPThenPlainAgain() async {
        await MainActor.run {
            ...
            // Phase 1: Plain text only (peer never used AXDP yet)
            viewModel.sessionManager.onDataReceived?(session, Data("Hello\r\n".utf8))
    ...
    /// Test flag state is correctly maintained through multiple switches
    func testFlagStateAcrossMultipleSwitches() async {
        await MainActor.run {
            ...
            // After plain text: flag cleared
            viewModel.sessionManager.onDataReceived?(session, Data("plain\r\n".utf8))
            XCTAssertFalse(viewModel.isPeerInAXDPReassembly(peerKey))
            ...
            // After plain text again: flag cleared again
            viewModel.sessionManager.onDataReceived?(session, Data("plain again\r\n".utf8))
            XCTAssertFalse(viewModel.isPeerInAXDPReassembly(peerKey))
            
            // Multiple AXDP in a row, then plain text
            ...
            viewModel.sessionManager.onDataReceived?(session, Data("final plain\r\n".utf8))
            XCTAssertFalse(viewModel.isPeerInAXDPReassembly(peerKey), "Flag should be cleared after plain text")
        }
    }
    ...
    func testTwoPeersWithDifferentProtocols() async {
        await MainActor.run {
            ...
            // Peer A switches to plain text
            viewModel.sessionManager.onDataReceived?(sessionA, Data("Hello from A\r\n".utf8))
            XCTAssertFalse(viewModel.isPeerInAXDPReassembly(keyA), "Peer A flag now cleared")
...
final class NonAXDPDeliveryIntegrationTests: XCTestCase {
    ...
    /// Simulate: AXDP negotiation happens, then user sends plain text
